...
def dataRegister(self: device.iotDevice):
    if self._logged == True:
        try:
            databaseOBJ.writeRaw("INSERT INTO log(id_device, input01, input02, input03, input04, date) values((select id from devices where tag = '"+self.get_tag()+"'), "+str(self.get_input01())+", "+str(self.get_input02())+", "+str(self.get_input03())+", "+str(self.get_input04())+", '"+str(now)+"')")
            return True
        except:
            return False
    else:
